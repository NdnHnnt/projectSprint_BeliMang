...
func UploadImage(c *fiber.Ctx) error {
	...
	if err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"message": "server error"})
	}

	files := form.File["file"]
	if len(files) == 0 {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"message": "No file is received"})
	}

	file := files[0]
	fileSize := file.Size
	if fileSize < 10*1024 || fileSize > 20*1024*1024 {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"message": "File size must be between 10 KB and 20 MB"})
	}

	fileType := file.Header.Get("Content-Type")
	if fileType != "image/jpeg" && fileType != "image/jpg" {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": "File must be a JPEG image"})
	}
	...
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"message": "Failed to decode image"})
	}
	...
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"message": "Failed to upload file to S3"})
	}
